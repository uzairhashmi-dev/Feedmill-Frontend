...
const Inventory = () => {
  ...
  const [triggerSearch] = useLazySearchInventoryItemsQuery();
  const [triggerFilter] = useLazyFilterInventoryItemsQuery();
  ...
  //  same save logic — call mutation, close on success
  const onSave = async () => {
    try {
      const success = editingId
        ? await updateInventoryItem({ id: editingId, formData }).unwrap()
  ...
  const onDeleteConfirm = async () => {
    ...
    try {
      await deleteInventoryItem(deletingItem._id).unwrap();
    } catch {
  ...
  const onFilterApply = useCallback(async (filters) => {
    const { data } = await triggerFilter(filters);
    setSearchResults(data ?? []);
  }, [triggerFilter]);

  const isSearching = searchResults !== null || searchTerm.trim().length > 0;
  const items = searchResults !== null ? searchResults : allItems;
