...
import {
  useGetCategoriesQuery, useLazySearchCategoriesQuery,
...
const Category = () => {
  ...
  const [createCategoryItem, { isLoading: creating }] = useCreateCategoryMutation();
  const [updateCategoryItem, { isLoading: updating }] = useUpdateCategoryMutation();
  const [deleteCategoryItem, { isLoading: deleting }]  = useDeleteCategoryMutation();
  const [triggerSearch] = useLazySearchCategoriesQuery();
  ...
  const onSubmit = async () => {
    ...
    try {
      const success = editingId
        ? await updateCategoryItem({ id: editingId, formData }).unwrap()
        : await createCategoryItem(formData).unwrap();
      if (success === true) {
        toast.success(
  ...
  const onDeleteConfirm = async () => {
    ...
    try {
      await deleteCategoryItem(deletingCat._id).unwrap();
    } catch {
      // error toast already shown by apiSlice
    }
    setShowDelete(false);
  ...
  const categories = searchResults !== null ? searchResults : allCategories;
